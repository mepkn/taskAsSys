...
export const checkDeadlinesAndSendReminders = async () => {
  ...
  try {
    ...
    console.log(
      `Found ${upcomingTasks.length} upcoming tasks. Sending reminders...`
    );

    for (const task of upcomingTasks) {
      for (const user of task.assignedTo as any) {
        // Type assertion to access populated fields
        if (user && user.email) {
          console.log(
            `Sending reminder for task "${task.title}" to ${user.email}`
          );
          await sendEmail({
            to: user.email,
            ...
            html: `<p>Hi ${
              user.name
            },</p><p>This is a reminder that the task "<strong>${
              task.title
            }</strong>" is due on <strong>${task.deadline.toDateString()}</strong>.</p><p>Please complete it on time.</p>`,
          });
        }
      }
